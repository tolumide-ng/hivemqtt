...
impl BufferIO for PubRel {
    fn length(&self) -> usize {
        ...
        if self.reason_code == PubRelReasonCode::Success && self.properties.length() == 0 { return len; }
        len += 1 + self.properties.length() + self.properties.variable_length(); // reason code
        len
    }

    fn write(&self, buf: &mut bytes::BytesMut) -> Result<(), MQTTError> {
        FixedHeader::new(Packet::PubRel, 0b10, self.length()).write(buf)?;

        self.packet_identifier.write(buf);
        if self.properties.length() == 0 && self.reason_code == PubRelReasonCode::Success { return Ok(()) }
    ...
    fn read_with_fixedheader(buf: &mut bytes::Bytes, header: FixedHeader) -> Result<Self, MQTTError> {
        ...
        packet.reason_code = PubRelReasonCode::try_from(u8::read(buf)?).map_err(|e| MQTTError::UnknownData(e))?;
...
mod tests {
    ...
    fn read_write_with_no_properties() {
        ...
        assert_eq!(header.packet_type, Packet::PubRel);
    ...
    fn read_write_with_neither_properties_nor_reasoncode() {
        ...
        assert_eq!(fixed_header.packet_type, Packet::PubRel);
    ...
    fn read_write_with_properties_and_reasoncode() {
        ...
        let expected = b"bB\0\0\x92>\x1f\0$thisIsAReasonStriing--andMoreAndMore&\0\rnotAuthorized\0\x05value".to_vec();
